...
public class mandelbrot {
	...
	public void draw(Graphics g, int width, int height, double zoom, double cx, double cy) {
		...
		// Bottom left
		xPoints[1] = centerX - size / 5;
		yPoints[1] = centerY + size / 2;
		
		// Bottom right
		xPoints[2] = centerX + size / 5;
		yPoints[2] = centerY + size / 2;
		
		// Draw recursively
		g2d.setColor(Color.WHITE);
		drawMandelbrot(g2d, xPoints, yPoints, 0);
	}
	
	private void drawMandelbrot(Graphics2D g, int[] x, int[] y, int depth) {
	    if (depth >= MAX_DEPTH) {
	    ...
	    // Top triangle
	    drawMandelbrot(g, new int[]{x[0], mx[0], mx[2]}, new int[]{y[0], my[0], my[2]}, depth + 1);
	    
	    // Bottom left triangle
	    drawMandelbrot(g, new int[]{mx[0], x[1], mx[1]}, new int[]{my[0], y[1], my[1]}, depth + 1);
	    
	    // Bottom right triangle
	    drawMandelbrot(g, new int[]{mx[2], mx[1], x[2]}, new int[]{my[2], my[1], y[2]}, depth + 1);
	}
}